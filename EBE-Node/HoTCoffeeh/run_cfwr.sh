...
for rf in 0.10
do
	direc=LARGE_results_`echo $rf`_rescaled_pT_grid
	rm -rf $direc
	for ((i=1; i<=1; i++))
	do
		mkdir results
		...
		nice -n 0 time bash ./HoTCoffeeh.sh false true CF_npphi=36 CF_npT=21 CF_npY=31 chosenParticlesMode=0 CF_resonanceThreshold=`echo $rf` qtnpts=13 qxnpts=11 qynpts=11 qznpts=11 delta_qx=0.02 delta_qy=0.02 delta_qz=0.02 &> ./results/all.out
