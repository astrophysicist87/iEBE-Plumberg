...
#do
	...
	do
		...
		#nice -n 0 time bash ./HoTCoffeeh.sh false true CF_npphi=48 CF_npT=15 CF_npY=15 chosenParticlesMode=0 CF_resonanceThreshold=`echo $rf` qtnpts=1 qxnpts=1 qynpts=1 qznpts=1 &> ./results/all.out
		#nice -n 0 time bash ./HoTCoffeeh.sh false true CF_npphi=48 CF_npT=15 CF_npY=15 chosenParticlesMode=0 CF_resonanceThreshold=`echo $rf` qtnpts=3 qxnpts=3 qynpts=3 qznpts=3 use_extrapolation=0 &> ./results/all.out
