...
int gauss_quadrature(int order, int kind, double alpha, double beta, double a, double b, double x[], double w[]) 
{
  ...
  switch(kind)
  {
     case 1: //Legendre,  1.0, integration interval: (a, b)
        ...
        beta = 0.0e0;
        break;
     case 2: //Chebyshev Type 1, ((b-x)*(x-a))^-0.5), integration interval: (a, b)
        alpha = 0.0e0;
        beta = 0.0e0;
        break;
     case 3: //Gegenbauer, ((b-x)*(x-a))^alpha  integration interval: (a, b)
     ...
     case 4: //Jacobi, (b-x)^alpha*(x-a)^beta integration interval: (a, b)
        ...
        break;
     case 5: //Generalized Laguerre, (x-a)^alpha*exp(-b*(x-a)) integration interval: (a,inf)
        //b is the scale factor in the exponential, and is typically 1.
        //alpha is a real parameter greater than -1
        alpha = 0.0e0;  //The generalized version does not work currently (Sept. 19, 2012)
        beta = 0.0e0;
        break;
     case 6: //Generalized Hermite, |x-a|^alpha*exp(-b*(x-a)^2) integration interval: (-inf, inf)
        ...
        break;
     case 7: // Exponential, |x-(a+b)/2.0|^alpha, integration interval: (a,b)
        //alpha is a real parameter greater than -1
        beta = 0.0e0;
        break;
     case 8: //Rational (x-a)^alpha*(x+b)^beta, integration interval: (a, inf)
        cout << " mode 8 is not support yet!" << endl;
        exit(1);
        break;
     case 9: //Chebyshev Type 2, ((b-x)*(x-a))^(+0.5), integration interval (a, b)
        alpha = 0.0e0;
        beta = 0.0e0;
        break;
     default:
  ...
  for(int i=0; i<order; i++)
  {
     switch(kind)
     {
        ...
        case 3: //Gegenbauer, ((b-x)*(x-a))^alpha  integration interval: (a, b)
           ...
           break;
        case 4: //Jacobi, (b-x)^alpha*(x-a)^beta integration interval: (a, b)
           w[i] = w[i]/(pow((b-x[i]), alpha)+eps)/(pow((x[i]-a), beta)+eps);
           break;
        case 5: //Generalized Laguerre, (x-a)^alpha*exp(-b*(x-a)) integration interval: (a,inf)
           w[i] = w[i]/(pow(fabs(x[i]-a), alpha)+eps)/(exp(-b*(x[i]-a))+eps);
           break;
        case 6: //Generalized Hermite, |x-a|^alpha*exp(-b*(x-a)^2) integration interval: (-inf, inf)
           w[i] = w[i]/(pow(fabs(x[i]-a), alpha)+eps)/(exp(-b*(x[i]-a)*(x[i]-a))+eps);
           break;
        case 7: // Exponential, |x-(a+b)/2.0|^alpha, integration interval: (a,b)
           w[i] = w[i]/(pow(fabs(x[i]-(a+b)/2.0), alpha)+eps);
           break;
        case 8: //Rational (x-a)^alpha*(x+b)^beta, integration interval: (a, inf)
           w[i] = w[i]/(pow((x[i]-a), alpha)+eps)/(pow((x[i]+b), beta)+eps);
           break;
        case 9: //Chebyshev Type 2, ((b-x)*(x-a))^(+0.5), integration interval (a, b)
...
int gauss_quadrature_standard(int order, int kind, double alpha, double beta, double x[], double w[]) 
{
