...
void CorrelationFunction::Cal_dN_dypTdpTdphi_with_weights_adjustable(int local_pid, int ipY, int iqt, int iqz, double * BC_chunk, int max_n_terms_to_compute, int local_part_mode)
{
	...
	if (MIDRAPIDITY_PIONS_ONLY)
		return;		//calculate pions at Y==0 separately;
					// only loop through this function if
					// doing pions at ALL rapidities like
...
void CorrelationFunction::Cal_dN_dypTdpTdphi_with_weights_Yeq0_adjustable(int iqt, int iqz, double * BC_chunk, int max_n_terms_to_compute)
{
	Stopwatch sw, sw_FOsurf;
	sw.Start();
